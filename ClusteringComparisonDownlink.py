...
for layout, file in enumerate(layout_files):
    ...
    for i, (processing, alg) in enumerate(cases):
        ...
        combiners_opt = network_opt.simulate_downlink_centralized(alg, collective_channels_opt,
                                                        collective_channel_estimates_opt)
        # combiners_opt = reshape_combiners(network_opt.simulate_downlink_distributed(alg, channels_opt,channel_estimates_opt)[0])
        average_SE_layout_i = network_opt.compute_downlink_SE_centralized(collective_channels_opt, combiners_opt)
        average_SE_opt[i, layout * num_users: (layout + 1) * num_users] = average_SE_layout_i

        combiners_emil = network_emil.simulate_downlink_centralized(alg, collective_channels_emil,
                                                        collective_channel_estimates_emil)
        # combiners_emil = reshape_combiners(network_opt.simulate_downlink_distributed(alg, channels_emil,channel_estimates_emil)[0])
        average_SE_layout_i = network_emil.compute_downlink_SE_centralized(collective_channels_emil, combiners_emil)
...
ax.set_xlabel(xlabel=f"Downlink Spectral Efficiency (bits/s/Hz)", fontsize=16)
