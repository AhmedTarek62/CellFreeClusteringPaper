...
from MyClusteringAlgorithms import general_chen
...
network_test = CellFreeNetwork(**params)
...
average_SE_ul_test = np.zeros((len(cases), num_layouts * num_users))
...
average_SE_dl_test = np.zeros((len(cases), num_layouts * num_users))
...
for layout in range(num_layouts):
    ...
    clusters_test = general_chen(path_loss_shadowing, num_pilots)
    ...
    for i, (processing, alg) in enumerate(cases):
        ...
        network_test.set_pilot_alloc(pilot_alloc_emil)
        network_test.set_clusters(clusters_test)
        network_test.set_snapshot(**layout_data_dict)
        ...
        collective_channels_test, collective_channel_estimates_test, _, _ = \
            network_test.generate_channel_realizations(num_frames)
        ...
        combiners_test = network_test.simulate_uplink_centralized(alg, collective_channels_test,
                                                        collective_channel_estimates_test)
        precoders_test = network_test.simulate_downlink_centralized(alg, collective_channels_test,
                                                        collective_channel_estimates_test)
        ...
        average_SE_layout_i = network_test.compute_uplink_SE_centralized(collective_channel_estimates_test, combiners_test)
        average_SE_ul_test[i, layout * num_users: (layout + 1) * num_users] = average_SE_layout_i

        average_SE_layout_i = network_test.compute_downlink_SE_centralized(collective_channels_test, precoders_test)
        average_SE_dl_test[i, layout * num_users: (layout + 1) * num_users] = average_SE_layout_i
...
average_SE_ul_test = np.sort(average_SE_ul_test, axis=1)
average_SE_ul_chen = np.sort(average_SE_ul_chen, axis=1)
average_SE_ul_opt = np.sort(average_SE_ul_opt, axis=1)

average_SE_dl_test = np.sort(average_SE_dl_test, axis=1)
...
index_90 = np.argmin(np.abs(np.linspace(0, 1, num_layouts * num_users) - 0.1))
...
sum_SE_test = average_SE_ul_test + average_SE_dl_test
X_axis = np.arange(len(algorithms))

fig_bar = plt.figure()
ax_bar = plt.subplot(111)
# ax_bar.grid(axis='y')
ax_bar.bar(X_axis - 0.25, sum_SE_opt[:,index_90], 0.2, label = 'Optimized', color='red')
ax_bar.bar(X_axis, sum_SE_chen[:,index_90], 0.2, label = 'Chen', color='blue')
ax_bar.bar(X_axis + 0.25, sum_SE_test[:,index_90], 0.2, label = 'Test', color='green')

ax_bar.set_xticks(X_axis, algorithms)
ax_bar.set_xlabel("Algorithm")
ax_bar.set_ylabel("90% likely sum SE")
ax_bar.legend()
plt.show()
test = []
