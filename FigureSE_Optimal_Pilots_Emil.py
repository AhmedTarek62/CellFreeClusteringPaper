...
# load optimized pilot emil solutions
...
for layout in range(num_layouts):
    ...
    pilots_emil = layout_data_dict['pilot_alloc_emil'].reshape((-1,))
    pilots_rand = np.random.randint(0, num_pilots, (num_users,))
    pilots = all_pilots_emil[layout]
    ...
    network.set_clusters(clusters_emil)
    ...
    network_rand.set_clusters(clusters_emil)
...
plt.savefig(f"Figures/{setting}_CDF_Pilots_Emil", dpi=800)
plt.show()
...
for p in pps_three:
    ...
    ax.text(x=p.get_x() + p.get_width() / 2, y=height+.10, s="{}".format(height), ha='center')
ax.tick_params(axis='both', labelsize=11)
plt.savefig(f"Figures/{setting}_CDF_min_pilots_emil", dpi=800)
plt.show()
...
for p in pps_two:
    ...
    ax.text(x=p.get_x() + p.get_width() / 2, y=height+.10, s="{}".format(height), ha='center')
ax.tick_params(axis='both', labelsize=11)
plt.savefig(f"Figures/{setting}_CDF_95_Pilots_Emil", dpi=800)
plt.show()
test = []
